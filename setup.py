...
if command == 'sdist':
    ...
    assert os.path.exists(VFILE), \
        'version.py does not exist, please set PISTON_VERSION (or run ' \
        'make_version.py for dev purposes)'
...
def load_pip_requires():
    path = os.path.join(os.path.dirname(__file__), 'tools', 'pip-requires')
...
setup(
    ...
    install_requires=load_pip_requires(),
    scripts=['bin/glance',
             'bin/glance-api',
